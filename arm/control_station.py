...
from kinematics import IK_numerical
...
class Gui(QMainWindow):
    ...
    def __init__(self, parent=None, dh_config_file=None):
        ...
        self.ui.videoDisplay.mousePressEvent = self.clickToPlace
        self.pickup = True
        ...
        self.ui.btnUser7.setText('Record Waypoint')
        self.ui.btnUser7.clicked.connect(partial(nxt_if_arm_init, 'record_waypoint'))
        self.ui.btnUser8.setText('Record Open Gripper')
        self.ui.btnUser8.clicked.connect(partial(nxt_if_arm_init, 'record_open_gripper'))
        self.ui.btnUser9.setText('Record Close Gripper')
        self.ui.btnUser9.clicked.connect(partial(nxt_if_arm_init, 'record_close_gripper'))
    ...
    def trackMouse(self, mouse_event):
        ...
        if self.camera.DepthFrameRaw.any() != 0:
            ...
            camera_coords = z * np.linalg.inv(self.camera.intrinsic_matrix).dot(np.array([[pt.x()], [pt.y()], [1]]))
            # print('camera_coords: ', camera_coords)
            # print('z: ', z)
            camera_coords_homogeneous = np.concatenate((camera_coords, np.array([[1]])), axis=0)
            world_coords = np.linalg.inv(self.camera.extrinsic_matrix).dot(camera_coords_homogeneous)

            self.ui.rdoutMouseWorld.setText("(%.0f,%.0f,%.0f)" %
                                            (world_coords[0][0], world_coords[1][0], world_coords[2][0]))

    def clickToPlace(self, mouse_event):
        """!
        ...
        if self.camera.DepthFrameRaw.any() != 0:
            ...
            # print('z: ', z)
            camera_coords_homogeneous = np.concatenate((camera_coords, np.array([[1]])), axis=0)
            world_coords = np.linalg.inv(self.camera.extrinsic_matrix).dot(camera_coords_homogeneous).flatten()[:3]

        dist_from_base = np.linalg.norm(world_coords - np.reshape(np.array([0, 0, 103.91]), world_coords.shape))
        if dist_from_base > 400:
            phi = 0
        else:
            phi = np.pi/2
        pose = np.concatenate((world_coords, np.array([phi])))

        if self.pickup:
            self.sm.set_next_state('pick', pose)
        else:
            self.sm.set_next_state('place', pose)
